...
class Oxides:
    ...
    def generate_dataset(self, number: int = 1) -> None:
        fixed_composition = ["Quartz"]
        generators = {
            "Quartz": self.create_mineral_data_fixed_composition,
        }
        ...
        for index in range(number):
            ...
            if self.name in fixed_composition:
                break
    ...
    def create_mineral_data_fixed_composition(self):
        ...
        if "constr_volume" not in self.cache[name_lower]:
            ...
            elif val_system == "trigonal":
                constr_vol = CrystalPhysics([[val_a, val_b], [], val_system])
            else:
    ...
    def create_mineral_data_variable_composition(self):
        ...
        for mineral in endmember:
            if mineral not in self.cache["endmembers"]:
                mineral_data = Oxides(name=mineral, random_seed=self.current_seed).generate_dataset(number=1)
